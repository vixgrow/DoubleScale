...
const Emails: React.FC<EmailsProps> = ({ contact_id }) => {
	...
	const fetchEmails = async () => {
		...
		try {
			...
			setTotal(response.messages.total || 0);
			setTotalRecords(response.messages.total || 0);
		} catch (error) {
			createNotice({
				type: 'error',
				message: __('Failed to fetch emails', 'quillcrm'),
	...
	return (
		<div className="qcrm-emails flex flex-col gap-5">
			...
			<div>
				{!loading &&
				(!emailAnalytics?.messages?.data ||
					emailAnalytics.messages.data.length === 0) ? (
					...
					<>
						<DataTable
							columns={columns}
							data={emailAnalytics?.messages?.data || []}
							loading={loading}
