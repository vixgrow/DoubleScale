...
final class Stream implements SeekableIterator
{
    ...
    {
        throw \League\Csv\UnavailableStream::dueToForbiddenCloning(self::class);
    }
    /**
     * {@inheritdoc}
     */
    ...
    public static function createFromPath(string $path, string $open_mode = 'r', $context = null) : self
    {
        ...
        if (!is_resource($resource)) {
            throw \League\Csv\UnavailableStream::dueToPathNotFound($path);
        }
    ...
    public function appendFilter(string $filtername, int $read_write, array $params = null) : void
    {
        ...
        if (!is_resource($res)) {
            throw \League\Csv\InvalidArgument::dueToStreamFilterNotFound($filtername);
        }
    ...
    private function filterControl(string $delimiter, string $enclosure, string $escape, string $caller) : array
    {
        if (1 !== strlen($delimiter)) {
            throw \League\Csv\InvalidArgument::dueToInvalidDelimiterCharacter($delimiter, $caller);
        }
        if (1 !== strlen($enclosure)) {
            throw \League\Csv\InvalidArgument::dueToInvalidEnclosureCharacter($enclosure, $caller);
        }
        if (1 === strlen($escape) || '' === $escape && 70400 <= PHP_VERSION_ID) {
            return [$delimiter, $enclosure, $escape];
        }
        throw \League\Csv\InvalidArgument::dueToInvalidEscapeCharacter($escape, $caller);
    }
    /**
     * Set CSV control.
     *
    ...
    {
        if (!$this->is_seekable) {
            throw \League\Csv\UnavailableFeature::dueToMissingStreamSeekability();
        }
        rewind($this->stream);
        $this->offset = 0;
        $this->value = \false;
    ...
    {
        if ($position < 0) {
            throw \League\Csv\InvalidArgument::dueToInvalidSeekingPosition($position, __METHOD__);
        }
        $this->rewind();
