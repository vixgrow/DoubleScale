...
namespace DoubleScale\Vendor\ArPHP\I18N;
...
class Arabic
{
    ...
    /** @var array<string> */
    private $arCommonChars = ['ة', 'ه', 'ي', 'ن', 'و', 'ت', 'ل', 'ا', 'س', 'م', 'e', 't', 'a', 'o', 'i', 'n', 's'];
    /** @var array<string> */
    ...
    /** @var boolean */
    private $stripTatweel = \true;
    /** @var boolean */
    private $stripTanween = \true;
    /** @var boolean */
    private $stripShadda = \true;
    /** @var boolean */
    private $stripLastHarakat = \true;
    /** @var boolean */
    private $stripWordHarakat = \true;
    /** @var boolean */
    private $normaliseLamAlef = \true;
    /** @var boolean */
    private $normaliseAlef = \true;
    /** @var boolean */
    private $normaliseHamza = \true;
    /** @var boolean */
    private $normaliseTaa = \true;
    /** @var array<string> */
    ...
    private function arDateInit()
    {
        if ($this->arDateLoad === \false) {
            ...
            $this->arDateJSON = \json_decode((string) \file_get_contents($file), \true);
            $this->arDateLoad = \true;
        }
    }
    /** @return void */
    ...
    private function arSpellerInit()
    {
        if ($this->arSpellerLoad === \false) {
            $this->speller = new \DoubleScale\Vendor\ArPHP\MZK\Speller();
            $this->arSpellerLoad = \true;
        }
    }
    /**
     * Spell Check
    ...
    {
        $this->arSpellerInit();
        $ret = $this->speller->spell_check($text, \true);
    ...
    private function arStandardInit()
    {
        ...
        $this->arStandardReplacements[] = 'DoubleScale\\Vendor\\ و\\1';
        /**
    ...
    private function arTransliterateInit()
    {
        if ($this->arTransliterateLoad === \false) {
            ...
            $json = \json_decode((string) \file_get_contents($file), \true);
    ...
    private function arGlyphsInit()
    {
        if ($this->arGlyphsLoad === \false) {
            ...
            $this->arGlyphs = \json_decode((string) \file_get_contents($file), \true);
            $this->arGlyphsLoad = \true;
        }
    }
    /** @return void */
    ...
    private function arSimilarityInit()
    {
        if ($this->arSimilarityLoad === \false) {
            $json = \json_decode((string) \file_get_contents($this->rootDirectory . '/data/ar_similarity.json'), \true);
     ...
     * @author Khaled Al-Sham'aa <khaled@ar-php.org>
     */
    private function arNumbersSubStr($number, $zero = \true)
    {
        $this->arNumbersInit();
        $blocks = [];
        $items = [];
        $zeros = '';
        $string = '';
        $number = $zero != \false ? \trim((string) $number) : \trim((string) (float) $number);
        if ($number > 0) {
     ...
     * @author Khaled Al-Sham'aa <khaled@ar-php.org>
     */
    public function addGlyphs($char, $hex, $prevLink = \true, $nextLink = \true)
    {
        $this->arGlyphsInit();
    ...
    private function arGlyphsPreConvert($str)
    {
        ...
        for ($i = $max - 1; $i >= 0; $i--) {
            ...
            if ($crntChar == 'ل' && isset($nextChar) && \mb_strpos('آأإا', $nextChar) !== \false) {
                $output = \substr($output, 0, \strlen($output) - 8);
                if (isset($this->arGlyphs[$prevChar]['prevLink']) && $this->arGlyphs[$prevChar]['prevLink'] == \true) {
                ...
                if ($prevChar == 'ل') {
                    if (isset($chars[$i - 2])) {
                        $tmp_form = isset($this->arGlyphs[$chars[$i - 2]]['prevLink']) && $this->arGlyphs[$chars[$i - 2]]['prevLink'] == \true ? 3 : 2;
                    } else {
                        $tmp_form = 2;
                    }
            ...
            if (\mb_strpos($this->arGlyphsVowel, $crntChar) !== \false) {
                if ($crntChar == 'ّ') {
                    if (\mb_strpos($this->arGlyphsVowel, $chars[$i - 1]) !== \false) {
                        ...
                        if ($prevChar && $this->arGlyphs[$prevChar]['prevLink'] == \true && ($nextChar && $this->arGlyphs[$nextChar]['nextLink'] == \true)) {
                            $form = 3;
                        }
            ...
            if ($prevChar && isset($this->arGlyphs[$prevChar]) && $this->arGlyphs[$prevChar]['prevLink'] == \true) {
                $form++;
            }
            // check if it should connect to the next char, the adjust the form value accordingly
            if ($nextChar && isset($this->arGlyphs[$nextChar]) && $this->arGlyphs[$nextChar]['nextLink'] == \true) {
                $form += 2;
            }
     ...
     * @author Khaled Al-Sham'aa <khaled@ar-php.org>
     */
    public function utf8Glyphs($text, $max_chars = 50, $hindo = \true, $forcertl = \false)
    {
        $lines = [];
        $pairs = [];
        $harakat = ['َ', 'ً', 'ُ', 'ٌ', 'ِ', 'ٍ'];
    ...
    private function arGlyphsDecodeEntities2($prefix, $codepoint, $original, &$table, &$exclude)
    {
        ...
        if ($codepoint < 0x80) {
            ...
            $str = \chr(0xe0 | (int) $codepoint >> 12) . \chr(0x80 | (int) $codepoint >> 6 & 0x3f) . \chr(0x80 | (int) $codepoint & 0x3f);
        } elseif ($codepoint < 0x200000) {
            $str = \chr(0xf0 | (int) $codepoint >> 18) . \chr(0x80 | (int) $codepoint >> 12 & 0x3f) . \chr(0x80 | (int) $codepoint >> 6 & 0x3f) . \chr(0x80 | (int) $codepoint & 0x3f);
        }
        // Check for excluded characters
        if (\in_array($str, $exclude, \true)) {
            return $original;
        } else {
            return $str;
        }
    ...
    private function getWordRegExp($arg)
    {
        ...
        $sql = \implode(" LIKE '%{$argLike}%' AND REGEXP '{$arg}') OR (", $this->arQueryFields) . " LIKE '%{$argLike}%' AND REGEXP '{$arg}'";
        /*
    ...
    private function arQueryAllWordForms($word)
    {
        ...
        if (\preg_match("/(\\S{1,})ئ(\\S{1,})/", $word) != \false) {
            foreach ($wordForms as $form) {
                $wordForms[] = \preg_replace("/(\\S{1,})ئ(\\S{1,})/", "DoubleScale\\Vendor\\1ي\\2", $form);
            }
        }
        $trans = ['أ' => 'ا', 'إ' => 'ا', 'آ' => 'ا'];
        foreach ($wordForms as $form) {
     ...
     * @author Khaled Al-Sham'aa <khaled@ar-php.org>
     */
    public function setSalatConf($sch = 'Shafi', $sunriseArc = -0.833333, $ishaArc = -17.5, $fajrArc = -19.5, $view = 'Sunni')
    {
        $sch = \ucfirst($sch);
    ...
    public function getPrayTime()
    {
        ...
        if ($this->salatSchool == 'Shafi') {
            $n = \sin(\atan(1 / (1 + \tan(\deg2rad($this->salatLat - $D))))) - \sin(\deg2rad($this->salatLat)) * \sin(\deg2rad($D));
        } else {
            $n = \sin(\atan(1 / (2 + \tan(\deg2rad($this->salatLat - $D))))) - \sin(\deg2rad($this->salatLat)) * \sin(\deg2rad($D));
        }
    ...
    public function getQibla()
    {
        ...
        $denominator = \cos(\deg2rad($latitude)) * \tan(\deg2rad($K_latitude)) - \sin(\deg2rad($latitude)) * \cos(\deg2rad($K_longitude - $longitude));
    ...
    public function arSummary($str, $keywords, $int, $mode, $output)
    {
        ...
        $summary = \trim(\preg_replace('/\\s+/', ' ', $summary));
        return $summary;
    }
    /**
    ...
    public function arSummaryKeywords($str, $int)
    {
        ...
        $normalizedStr = $this->arNormalize($str);
        $cleanedStr = $this->arCleanCommon($normalizedStr);
        $str = \preg_replace('/(\\W)ال(\\w{3,})/u', 'DoubleScale\\Vendor\\1\\2', $cleanedStr);
        $str = \preg_replace('/(\\W)وال(\\w{3,})/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})هما(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})كما(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})تين(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})هم(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})هن(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})ها(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})نا(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})ني(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})كم(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})تم(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})كن(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})ات(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})ين(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})تن(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})ون(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})ان(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})تا(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})وا(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
        $str = \preg_replace('/(\\w{3,})ة(\\W)/u', 'DoubleScale\\Vendor\\1\\2', $str);
    ...
    private function arSummaryRankSentences($sentences, $stemmedSentences, $arr)
    {
        ...
        for ($i = 0; $i < $max; $i++) {
            ...
            if ($first == "\n") {
                $w += 3;
            } elseif (\in_array($first, $this->arSeparators, \true)) {
                $w += 2;
            } else {
                $w += 1;
            }
            if ($last == "\n") {
                $w += 3;
            } elseif (\in_array($last, $this->arSeparators, \true)) {
                $w += 2;
            } else {
                $w += 1;
            }
            ...
            if (!\in_array($first, $this->arSeparators, \true)) {
     ...
     * @author Khaled Al-Sham'aa <khaled@ar-php.org>
     */
    public function arIdentify($str, $html = \true)
    {
    ...
    public function arPlural($singular, $count, $plural2 = null, $plural3 = null, $plural4 = null, $nameOnly = \false, $isFemale = null)
    {
        ...
        if ($nameOnly) {
            $plural = \trim(\str_replace('%d', '', $plural));
        }
        return $plural;
    }
    /**
     ...
     * @author Khaled Al-Sham'aa <khaled@ar-php.org>
     */
    public function stripHarakat($text, $tatweel = \true, $tanwen = \true, $shadda = \true, $last = \true, $harakat = \true)
    {
        $text = $this->setNorm('stripTatweel', $tatweel)->setNorm('stripTanween', $tanwen)->setNorm('stripShadda', $shadda)->setNorm('stripLastHarakat', $last)->setNorm('stripWordHarakat', $harakat)->setNorm('normaliseLamAlef', \true)->setNorm('normaliseAlef', \false)->setNorm('normaliseHamza', \false)->setNorm('normaliseTaa', \false)->arNormalizeText($text);
        return $text;
    }
    /**
    ...
    public function arSentiment($text)
    {
        ...
        $text = $this->setNorm('normaliseAlef', \true)->setNorm('normaliseHamza', \true)->setNorm('normaliseTaa', \true)->arNormalizeText($text);
        ...
        $negationWords = ['لا', 'ليس', 'غير', 'ما', 'لم', 'لن', 'لست', 'ليست', 'ليسا', 'ليستا', 'لستما', 'لسنا', 'لستم', 'ليسوا', 'لسن', 'لستن'];
        $negationFlag = \false;
        # for each word
        foreach ($words as $word) {
    ...
    public function noDots($text)
    {
        ...
        $text = \strtr($text, ['ب' => 'ٮ', 'ت' => 'ٮ', 'ث' => 'ٮ', 'ن' => 'ٮ', 'ي' => 'ى', 'ف' => 'ڡ', 'ق' => 'ٯ', 'ش' => 'س', 'غ' => 'ع', 'ذ' => 'د', 'ز' => 'ر', 'ض' => 'ص', 'ظ' => 'ط', 'ة' => 'ه', 'ج' => 'ح', 'خ' => 'ح', 'أ' => 'ا', 'إ' => 'ا', 'آ' => 'ا', 'ؤ' => 'و', 'ئ' => 'ى']);
        return $text;
    }
    /////////////////////////////////////// Normalize ///////////////////////////////////////////////
    /**
     ...
     * @author Khaled Al-Sham'aa <khaled@ar-php.org>
     */
    public function diffForHumans($time, $others = null, $parts = 2, $floor = \true)
    {
        $diff = $others == null ? $time - \time() : $time - $others;
        if ($diff < 0) {
