 ...
 */
import './style.scss';
import type { DataTableConfig, NoticeMessage } from '@quillcrm/client';
import { NoticeBanner, NoData, ToolsIcon } from '@quillcrm/components';
...
const Rules = forwardRef<RulesRef, RulesProps>(({ activeTab }, ref) => {
	...
	const fetchRules = async () => {
		...
		try {
			const response = (await apiFetch({
				path: addQueryArgs('/qc/v1/lead-scoring-rules', {
					per_page: perPage,
					page,
	...
	const validate = (rule: Partial<LeadScoringRule>) => {
		if (!rule.title || rule.title.trim() === '') {
			setVisible(false);
			showNotice('error', __('Rule title is required', 'quillcrm'));
			return false;
		}
		if (rule.points === undefined || rule.points < 0) {
			...
			showNotice(
				'error',
				__('Points must be a positive number', 'quillcrm')
			);
			return false;
		}
		return true;
	...
	const createRule = async () => {
		...
		try {
			...
			setVisible(false);
			await fetchRules();
			showNotice('success', __('Rule created successfully', 'quillcrm'));
		} catch (error: any) {
			setVisible(false);
			showNotice('error', error.message);
		} finally {
	...
	const updateRule = async () => {
		...
		try {
			const response = (await apiFetch({
				path: `/qc/v1/lead-scoring-rules/${selectedRule?.id}`,
				method: 'PUT',
				data: selectedRule,
			})) as LeadScoringRule;
			...
			setVisible(false);
			setSelectedRule(null);
			showNotice('success', __('Rule updated successfully', 'quillcrm'));
		} catch (error: any) {
			setVisible(false);
			showNotice('error', error.message);
		} finally {
	...
	const deleteSelectedRules = async () => {
		...
		try {
			...
			showNotice(
				'success',
				__('Selected rules deleted successfully', 'quillcrm')
			);
		} catch (error: any) {
			showNotice('error', error.message);
		} finally {
	...
	const tableConfig: DataTableConfig<LeadScoringRule> = {
		...
		search: {
			placeholder: __('Search Rules', 'quillcrm'),
	...
	return (
		<div className="qcrm-lead-scoring-rules-list">
			{/* Notice Banner */}
			{notice && (
				<NoticeBanner
					ref={noticeBannerRef}
			...
			{loading || hasRecords ? (
				...
				<NoData
					...
					title={__('No Rules Found', 'quillcrm')}
					subtitle={__(
						'Create your first lead scoring rule to get started',
						'quillcrm'
					)}
					buttonLabel={__('Add Rule', 'quillcrm')}
					onClick={() => {
						setSelectedRule(null);
						setRule({
							title: '',
