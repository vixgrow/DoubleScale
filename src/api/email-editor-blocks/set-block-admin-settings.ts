 ...
 */
import { BlockAdminSettings } from '../../stores/email-editor-blocks/types.js';
...
export const setBlockAdminSettings = (
	...
	const blockType = select('quillcrm/email-editor-blocks').getBlockType(name);
	if (!blockType) {
		console.error(
	...
	dispatch('quillcrm/email-editor-blocks').setBlockAdminSettings(
		pick(settings, [
			'controls',
			'edit'
		]),
