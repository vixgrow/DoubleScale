                ...
                <div className="absolute inset-0 z-20 bg-white/90 backdrop-blur-sm flex items-center justify-center">
                    <div className="text-center p-4">
                        ...
                        <p className="text-sm text-gray-600 mb-3">
                            {__('Upgrade to QuillCRM Pro to unlock this integration', 'quillcrm')}
                        </p>
                        <Button
                            variant="default"
                            size="sm"
                            className="rounded-lg"
                            onClick={() => window.open('https://quillcrm.com/pricing', '_blank')}
                        >
            ...
            <CardContent className={`p-4 ${isLoading ? 'opacity-50' : ''}`}>
                ...
                <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-4">
                        ...
                        <div className={`font-semibold text-xl ${isProFeature ? 'blur-sm' : ''}`}>
                            {integration.label}
                        </div>
                    </div>
                ...
                <div className={`text-base text-gray-500 border-b pb-3 ${isProFeature ? 'blur-sm' : ''}`}>
                    {integration.description}
                </div>

                {/* Action buttons - Blur only this section for Pro features */}
                <div className={`flex items-center justify-end gap-3 mt-4 ${isProFeature ? 'blur-sm' : ''}`}>
                    <Button
                        onClick={onNavigate}
                        variant="secondary"
                        className="rounded-lg"
