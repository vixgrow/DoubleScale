 ...
 * Email_Clicked class
 */
class Email_Clicked extends Filter {
	...
	public function apply( Builder $query, $filter = array() ) {
		...
		$table_tracking = $wpdb->prefix . 'quillcrm_communication_tracking';
		...
		$query->whereRaw(
			...
			array_merge(
				array( Communication_Tracking_Model::MODE_EMAIL ),
				$time_bindings,
				array( $count_value )
			)
		);
...
Filters_Manager::instance()->register( new Email_Clicked() );
