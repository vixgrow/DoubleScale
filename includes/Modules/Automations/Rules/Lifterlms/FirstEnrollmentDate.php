 ...
 *
 * This class is responsible for handling the first enrollment date rule for LifterLMS
 *
 * @since 1.0.0
 *
...
class FirstEnrollmentDate extends Rule
{
	 ...
	 * @since 1.0.0
	 */
	public $slug = 'lifterlms_first_enrollment_date';
					...
					foreach ($courses['results'] as $course_id) {
						...
						if ($enrollment_date) {
							...
							if ($timestamp && ($first_date === null || $timestamp < strtotime($first_date))) {
								$first_date = date('Y-m-d', $timestamp);
							}
						}
					}
				}
