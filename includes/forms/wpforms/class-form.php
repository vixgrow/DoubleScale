...
use QuillCRM\Forms\WPForms\Form_Utils;
...
class Form extends Abstracts_Form {
	...
	public function prepare_fields( $fields ) {
		...
		foreach ( $fields as $field ) {
			...
			$field_id = Form_Utils::get_field_id( $field['id'] );
			$label    = $field['label'] ?? $field['name'];

			switch ( $field['type'] ) {
				case 'name':
					$fields_arr = array_merge( $fields_arr, Form_Utils::prepare_name_field( $field, $field_id ) );
					break;
				case 'address':
	...
	public function ajax_get_fields() {
		...
		if ( ! $form_id ) {
			wp_send_json_error( array( 'message' => __( 'Form ID is required.', 'quill-crm' ) ) );
		}
