...
export const ANALYTICS_ACTIONS = ['send_email', 'send_sms', 'send_whatsapp'] as const;

export type AnalyticsActionType = typeof ANALYTICS_ACTIONS[number];

export type ActionChannelType = 'email' | 'sms' | 'whatsapp';
...
export const getChannelType = (action: string): ActionChannelType => {
	...
	if (action === 'send_whatsapp') return 'whatsapp';
