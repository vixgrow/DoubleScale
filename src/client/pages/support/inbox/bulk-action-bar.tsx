...
const ActionButton: React.FC<ActionButtonProps> = ({
	...
	<button
		...
		onClick={onClick}
		disabled={disabled}
		className={`flex min-w-[4rem] flex-col items-center gap-1 rounded-lg px-2 py-1.5 transition-colors disabled:cursor-not-allowed disabled:opacity-40 sm:min-w-[4.5rem] ${
			destructive
...
const BulkActionBar: React.FC<BulkActionBarProps> = ({
	...
	return (
		<div className="fixed bottom-3 left-2 right-2 z-[100] sm:left-1/2 sm:right-auto sm:top-20 sm:bottom-auto sm:-translate-x-1/2">
			<div className="overflow-x-auto rounded-xl border border-gray-700 bg-gray-900 px-2 py-2 shadow-2xl">
				<div className="flex min-w-max items-center gap-1">
					<div className="mr-1 flex items-center gap-1 border-r border-gray-700 pr-2">
						<span className="hidden whitespace-nowrap px-2 text-sm font-medium text-white sm:inline">
						{selectedCount === 1
						...
						<button
							...
							onClick={onClear}
							disabled={busy}
							className="rounded-md p-1 text-gray-400 hover:bg-white/10 hover:text-white disabled:opacity-40"
							title={__('Clear selection', 'doublescale')}
							aria-label={__('Clear selection', 'doublescale')}
						>
