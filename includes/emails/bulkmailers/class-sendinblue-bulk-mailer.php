...
/**
 * Sendinblue_Bulk_Mailer class
 *
 * @since 1.0.0
 */
class Sendinblue_Bulk_Mailer extends Abstract_Bulk_Mailer {
	...
	public function send_batch( $batch_data ) {
		...
		// Get Sendinblue/Brevo Account API
		$account_api = $this->get_account_api();

		if ( ! $account_api ) {
			return new WP_Error(
				'sendinblue_not_configured',
				__( 'Sendinblue/Brevo is not properly configured.', 'quillcrm' )
			);
		}
		...
		if ( ! method_exists( $account_api, 'send_batch' ) ) {
			return new WP_Error(
				...
				__( 'Sendinblue/Brevo batch sending is not available. Please update QuillSMTP.', 'quillcrm' )
			);
		}
