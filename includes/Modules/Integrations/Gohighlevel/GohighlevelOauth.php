...
class GohighlevelOauth {
	...
	public static function get_authorization_url( $client_id ) {
		...
		set_transient( 'doublescale_ghl_temp_client_id', $client_id, 600 ); // 10 minutes
	...
	public function maybe_authorize() {
		$action = $_GET['doublescale-ghl'] ?? $_GET['ds-ghl'] ?? null;
		if ( $action !== 'authorize' ) {
			return;
		}
		...
		set_transient( 'doublescale_ghl_temp_client_secret', $client_secret, 600 ); // 10 minutes
