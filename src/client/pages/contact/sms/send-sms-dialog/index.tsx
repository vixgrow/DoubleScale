...
const SendSMSDialog: React.FC<SendSMSDialogProps> = ({
    ...
    const sendSMS = async () => {
        ...
        if (toPhone.length < 10) {
            createNotice({
                type: 'error',
                message: __('Please enter a valid phone number (E.164 format: +1234567890)', 'quillcrm'),
            });
            return;
        }
        ...
        try {
            const response = await apiFetch({
                path: `/qc/v1/contacts/${contact.id}/send-sms`,
                method: 'POST',
                data: {
                    to: toPhone,
                    message: message,
            ...
            <DialogContent className="max-w-[500px] z-[1700000]">
                ...
                <div className="flex flex-col gap-4">
                    <Field
                        ...
                        placeholder={__('Enter phone number (+1234567890)', 'quillcrm')}
                    ...
                    <div>
                        <Field
                            ...
                            placeholder={__('Enter your message...', 'quillcrm')}
                            value={message}
                        ...
                        <div className="flex justify-between text-xs text-gray-500 mt-1">
                            ...
                            <span>
                                {smsCount} SMS {smsCount === 1 ? __('segment', 'quillcrm') : __('segments', 'quillcrm')}
                            </span>
                        </div>
                    </div>
                </div>
