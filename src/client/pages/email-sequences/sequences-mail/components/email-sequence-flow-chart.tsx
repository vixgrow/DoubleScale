...
import FlagIcon from '@doublescale/components/icons/flag';
import TrashIcon from '@doublescale/components/icons/trash';
import SentSTimeIcon from '@doublescale/components/icons/send-time';
import EmailSubjectIcon from '@doublescale/components/icons/email-subject';
			...
			<EdgeLabelRenderer>
				...
				>
					<button
						...
						title={__('Add email here', 'doublescale')}
					>
						<PlusIcon />
					</button>
				</div>
		...
		>
			...
			<div className="flex items-center justify-between py-2 px-3 mb-3 border border-[#DEE1E6] rounded-t-lg bg-[#F8F8F8]">
				<span className="text-sm font-semibold text-[#09090B]">
					{__(`Step ${nodeData.number}`, 'doublescale')}
				</span>
		...
		>
			...
			<div className="flex items-center gap-3">
				...
				<span className="text-xl font-bold leading-[30px] text-[#09090B]">
					{__('Start (Email Sequence)', 'doublescale')}
				</span>
			</div>
		</div>
	);
...
const EmailSequenceFlowChart: React.FC<EmailSequenceFlowChartProps> = ({
	...
	const formatDelay = (delay?: { value: number; unit: string }) => {
		if (!delay || delay.value === 0) return __('Immediately', 'doublescale');
		...
		return (
			<div className="h-full flex items-center justify-center bg-[#F6F8FA]">
				<p className="text-xl text-gray-600">
					{__('Loading sequence...', 'doublescale')}
				</p>
			</div>
		);
	}
