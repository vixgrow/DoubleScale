...
abstract class Abstract_Campaign_Processing
{
    ...
    {
        return Campaign_Model::where(function($query) {
     ...
     */
    protected function get_template_for_contact(Campaign_Model $campaign, Contact_Model $contact)
    {
    ...
    protected function validate_template($template, $campaign_type)
    {
        ...
        if ($campaign_type === 'email' && strpos($template->body, '{{contact:unsubscribe_link}}') === false) {
            quillcrm_get_logger()->warning(
                ...
                array('template_id' => $template->id, 'template_name' => $template->name ?? 'Unknown')
            );
        }
        
        // Validate content length for SMS/WhatsApp
        if (in_array($campaign_type, ['sms', 'whatsapp'])) {
            $plain_text = wp_strip_all_tags($template->body);
            if (strlen($plain_text) > 1600) { // SMS limit is typically 1600 chars
                quillcrm_get_logger()->warning(
                    sprintf(__('%s template content may be too long (%d characters)', 'quillcrm'), ucfirst($campaign_type), strlen($plain_text)),
                    array('template_id' => $template->id, 'length' => strlen($plain_text))
                );
            }
        }
