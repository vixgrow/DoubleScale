...
class Admin {
	...
	public function create_admin_menu_pages() {
		...
		// page under the default slug so old bookmarks/links don't 403.
		// Conversely, when the slug IS the default, an addon may register its
		// custom slug as an alias via the doublescale_admin_menu_alias hook.
		if ( $menu_slug !== $default_slug ) {
			add_submenu_page(
				null,
				'',
				'',
				'doublescale_access',
				$default_slug,
				array( AdminLoader::class, 'page_wrapper' )
			);
		}

		$alias_slug = apply_filters( 'doublescale_admin_menu_alias', '' );
		if ( $alias_slug && $alias_slug !== $menu_slug ) {
			add_submenu_page(
				null,
				'',
				'',
		...
		$default_icon = 'data:image/svg+xml;base64,' . base64_encode(
			'<svg xmlns="http://www.w3.org/2000/svg" width="31" height="40" viewBox="0 0 31 40" fill="none">
				...
				<path d="M11.609 4.21103C13.1768 3.96352 16.0576 4.30087 17.6276 4.51679C16.1495 5.9252 15.5263 6.53766 14.3241 8.23588C6.07539 8.18097 4.32991 12.6278 5.14316 20.4084C5.33029 22.1984 6.15794 23.5259 7.23921 24.9352C7.26533 26.8033 7.488 27.8211 8.08354 29.5638C8.22048 29.8479 8.20953 29.8593 8.29373 30.1689L8.10966 30.3026C6.97029 30.2733 4.9038 28.9842 4.05029 28.2593C1.62431 26.1398 0.426352 23.0289 0.119345 19.8992C-0.702438 11.522 2.67615 4.99295 11.609 4.21103Z" fill="white"/>
		...
		foreach ( MenuRegistry::all( MenuRegistry::builtin_entries( $menu_slug ) ) as $item ) {
			add_submenu_page(
				$menu_slug,
				isset( $item['page_title'] ) ? $item['page_title'] : '',
				isset( $item['menu_title'] ) ? $item['menu_title'] : '',
				isset( $item['capability'] ) ? $item['capability'] : 'doublescale_access',
				isset( $item['slug'] ) ? $item['slug'] : $menu_slug,
				isset( $item['callback'] ) ? $item['callback'] : array( AdminLoader::class, 'page_wrapper' )
			);
		}
	}
}
