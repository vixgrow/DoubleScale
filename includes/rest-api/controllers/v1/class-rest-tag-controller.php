...
class REST_Tag_Controller extends REST_Controller {
	...
	public function get_items( $request ) {
		try {
			...
			$query       = Tag_Model::query();
			$total_count = $query->count();

			if ( ! empty( $ids ) ) {
				$tags = $query->whereIn( 'id', $ids )->orderBy( 'created_at', 'desc' )->paginate( $per_page, array( '*' ), 'page', $page );

				return new WP_REST_Response( $tags, 200 );
			}

			if ( '' !== $keyword ) {
				$tags = $query->where( 'name', 'LIKE', '%' . $keyword . '%' )
					->orWhere( 'description', 'LIKE', '%' . $keyword . '%' );
			}

			if ( '' !== $contact_id ) {
				$tags = $query->whereDoesntHave(
					'contacts',
			...
			$tags = $query->orderBy( 'created_at', 'desc' )->paginate( $per_page, array( '*' ), 'page', $page );

			return new WP_REST_Response( $tags->toArray() + array( 'total_count' => $total_count ), 200 );
		} catch ( \Exception $e ) {
