/**
 * Toolbar-agnostic Lexical editor shell.
 *
 * Owns the Lexical composer setup (nodes, theme, plugins, HTML (de)serialization,
 * external-content syncing, word count) but renders whatever toolbar the caller
 * injects. This is the seam that keeps the bundle clean: the email editor
 * ({@see Editor}) injects the heavy `EmailToolbar` (merge tags → admin tree),
 * while the support composer ({@see SupportRichText}) injects the import-clean
 * `SupportToolbar`. Because this shell imports NEITHER toolbar, the public
 * support portal bundle never pulls `@doublescale/components`.
 */
...
export interface EditorShellProps {
	...
	/** Toolbar rendered inside the Lexical composer (injected per variant). */
