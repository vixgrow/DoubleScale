...
namespace DoubleScale\Vendor\Dompdf\Adapter;

use DoubleScale\Vendor\Dompdf\Canvas;
use DoubleScale\Vendor\Dompdf\Dompdf;
use DoubleScale\Vendor\Dompdf\Exception;
use DoubleScale\Vendor\Dompdf\FontMetrics;
use DoubleScale\Vendor\Dompdf\Helpers;
use DoubleScale\Vendor\Dompdf\Image\Cache;
use DoubleScale\Vendor\FontLib\Exception\FontNotFoundException;
/**
...
class CPDF implements Canvas
{
    ...
    public function __construct($paper = "letter", string $orientation = "portrait", ?Dompdf $dompdf = null)
    {
        ...
        $this->_pdf = new \DoubleScale\Vendor\Dompdf\Cpdf($size, \true, $this->_dompdf->getOptions()->getFontCache(), $this->_dompdf->getOptions()->getTempDir());
    ...
    public function select($x, $y, $w, $h, $font, $size, $color = [0, 0, 0], $opts = [])
    {
        ...
        $ft = \DoubleScale\Vendor\Dompdf\Cpdf::ACROFORM_FIELD_CHOICE;
        $ff = \DoubleScale\Vendor\Dompdf\Cpdf::ACROFORM_FIELD_CHOICE_COMBO;
    ...
    public function textarea($x, $y, $w, $h, $font, $size, $color = [0, 0, 0])
    {
        ...
        $ft = \DoubleScale\Vendor\Dompdf\Cpdf::ACROFORM_FIELD_TEXT;
        $ff = \DoubleScale\Vendor\Dompdf\Cpdf::ACROFORM_FIELD_TEXT_MULTILINE;
    ...
    public function input($x, $y, $w, $h, $type, $font, $size, $color = [0, 0, 0])
    {
        ...
        switch ($type) {
            ...
            case 'password':
                $ft = \DoubleScale\Vendor\Dompdf\Cpdf::ACROFORM_FIELD_TEXT;
                $ff = \DoubleScale\Vendor\Dompdf\Cpdf::ACROFORM_FIELD_TEXT_PASSWORD;
                break;
            case 'submit':
                $ft = \DoubleScale\Vendor\Dompdf\Cpdf::ACROFORM_FIELD_BUTTON;
                break;
        }
        $pdf->addFormField($ft, \rand(), $x, $this->y($y) - $h, $x + $w, $this->y($y), $ff, $size, $color);
    }
