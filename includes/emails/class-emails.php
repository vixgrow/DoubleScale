...
class Emails {
	...
	public function __construct() {
		...
		add_action( 'quillcrm_email_send_before', array( $this, 'send_before' ) );
		add_action( 'quillcrm_email_send_after', array( $this, 'send_after' ) );
	}
	...
	public function get_from_name() {
		...
		return apply_filters( 'quillcrm_email_from_name', quillcrm_decode_string( $this->from_name ), $this );
	}
	...
	public function get_from_address() {
		...
		return apply_filters( 'quillcrm_email_from_address', quillcrm_decode_string( $this->from_address ), $this );
	}
	...
	public function get_reply_to() {
		...
		return apply_filters( 'quillcrm_email_reply_to', quillcrm_decode_string( $this->reply_to ), $this );
	}
	...
	public function get_cc() {
		...
		return apply_filters( 'quillcrm_email_cc', quillcrm_decode_string( $this->cc ), $this );
	}
	...
	public function get_content_type() {
		...
		if ( ! $this->content_type && $this->html ) {
			$this->content_type = apply_filters( 'quillcrm_email_default_content_type', 'text/html', $this );
		} elseif ( ! $this->html ) {
			$this->content_type = 'text/plain';
		}

		return apply_filters( 'quillcrm_email_content_type', $this->content_type, $this );
	}
	...
	public function get_headers() {
		...
		return apply_filters( 'quillcrm_email_headers', $this->headers, $this );
	}
	...
	public function build_email( $message ) {
		...
		if ( false === $this->html ) {
			return apply_filters( 'quillcrm_email_message', quillcrm_decode_string( $message ), $this );
		}
	...
	public function send( $to, $subject, $message, $attachments = array() ) {
		...
		do_action( 'quillcrm_email_send_before', $this );
		 ...
		 */
		$data = apply_filters(
			'quillcrm_emails_send_email_data',
			array(
				'to'          => $to,
				'subject'     => $subject,
				'message'     => $message,
		...
		do_action( 'quillcrm_email_send_after', $this );
	...
	public function get_template() {
		...
		return apply_filters( 'quillcrm_email_template', $this->template );
	}
	...
	public function get_theme_template_paths() {
		...
		$template_dir = 'quillcrm-email';

		$file_paths = array(
			1   => trailingslashit( get_stylesheet_directory() ) . $template_dir,
			10  => trailingslashit( get_template_directory() ) . $template_dir,
			100 => QUILLCRM_PLUGIN_DIR . 'includes/emails/templates',
		);

		$file_paths = apply_filters( 'quillcrm_email_template_paths', $file_paths );
	...
	private function get_prepared_subject( $subject ) {
		...
		return quillcrm_decode_string( $subject );
	}
}
