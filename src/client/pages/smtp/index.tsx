...
const SmtpPage: React.FC = () => {
	...
	const [alertsDraft, setAlertsDraft] = useState<SmtpAlertsSettings | null>(
		null
	);
	...
	const [smtpConnectionsView, setSmtpConnectionsView] = useState<
		'table' | 'card'
	>('table');
	...
	const smtpSectionTitles: Record<string, string> = {
		settings: __('Connections', 'doublescale'),
						...
						? [
								{
									...
									onClick: () =>
										settingsAddConnectionRef.current?.(),
	...
	return (
		<div className="smtp-hub space-y-6">
			{section === 'settings' ? (
				...
				<PageHeader
					title={smtpSectionTitles[section] ?? section}
...
const SmtpEmailTest: React.FC<{
	...
	const send = async () => {
		if (!email || !connection) {
			setMsg({
				type: 'err',
				text: __(
					'Choose a connection and enter a recipient email.',
					'doublescale'
				),
			});
			return;
		...
		try {
			...
			setMsg({
				type: 'ok',
				text: __('Email sent successfully.', 'doublescale'),
			});
		} catch (e: unknown) {
	...
	return (
		<div className="w-full flex flex-col bg-white gap-6 min-h-screen p-6 rounded-[20px] bg-background shadow-[0_4px_20px_0_rgba(59,130,246,0.14)]">
			<div className="flex items-start gap-2">
				<EmailTestIcon />
				<div className="space-y-2">
					<h3 className="text-xl font-semibold leading-8 text-foreground">
						{__('Test Your Email Configuration', 'doublescale')}
					</h3>
			...
			<div className="space-y-4">
				...
				<div className="flex items-center gap-2">
					...
					<Label
						...
						htmlFor="smtp-test-html"
					>
						{__(
							'HTML Email (Send the email as HTML or plain text.)',
							'doublescale'
						)}
					</Label>
				</div>
				{isHtml ? (
					...
					<div className="space-y-2">
						<Label
							className="text-foreground"
							htmlFor="smtp-test-plain-message"
						>
