...
class Writer extends \League\Csv\AbstractCsv
{
    ...
    public function insertOne(array $record) : int
    {
        ...
        if (\false === $bytes || 0 >= $bytes) {
            throw \League\Csv\CannotInsertRecord::triggerOnInsertion($record);
        }
    ...
    protected function validateRecord(array $record) : void
    {
        foreach ($this->validators as $name => $validator) {
            if (\true !== $validator($record)) {
                throw \League\Csv\CannotInsertRecord::triggerOnValidation($name, $record);
            }
        }
    }
    /**
    ...
    public function setFlushThreshold(?int $threshold) : self
    {
        ...
        if (null !== $threshold && 1 > $threshold) {
            throw \League\Csv\InvalidArgument::dueToInvalidThreshold($threshold, __METHOD__);
        }
