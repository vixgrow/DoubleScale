...
export const useCustomFields = (scope: string = 'contact') => {
	...
	const fetchGroups = async () => {
		try {
			...
			const response = (await apiFetch({
				path: addQueryArgs('/doublescale/v1/custom-fields-groups', {
					scope,
				}),
			})) as CustomFieldsGroups;

			if (!Array.isArray(response)) {
				throw new Error(__('Invalid response format', 'doublescale'));
			}
			...
			showNotice({
				...
				message:
					error.message ||
					__('Failed to load custom fields', 'doublescale'),
			});
		}
	};
	...
	const saveField = async (field: CustomField, isNew: boolean) => {
		try {
			...
			const path = isNew
				? '/doublescale/v1/custom-fields'
				: `/doublescale/v1/custom-fields/${field.id}`;
			...
			showNotice({
				type: 'success',
				message: isNew
					? __('Custom field added', 'doublescale')
					: __('Custom field updated', 'doublescale'),
			});
			setIsUpdated(true);
			return true;
		} catch (error: any) {
	...
	const deleteField = async (field: CustomField) => {
		try {
			await apiFetch({
				path: `/doublescale/v1/custom-fields/${field.id}`,
				method: 'DELETE',
			});
			...
			showNotice({
				type: 'success',
				message: __('Field deleted', 'doublescale'),
			});
		} catch (error: any) {
			showNotice({
				type: 'error',
	...
	const deleteSelectedFields = async (fieldIds: number[]) => {
		try {
			await apiFetch({
				path: '/doublescale/v1/custom-fields',
				method: 'DELETE',
				data: { ids: fieldIds },
			});
			...
			showNotice({
				type: 'success',
				message: __('Selected fields deleted', 'doublescale'),
			});
		} catch (error: any) {
			showNotice({
				type: 'error',
	...
	const saveGroup = async (name: string) => {
		try {
			const response = (await apiFetch({
				path: '/doublescale/v1/custom-fields-groups',
				method: 'POST',
				data: {
					name,
					scope,
			...
			showNotice({
				type: 'success',
				message: __('Group added', 'doublescale'),
			});
			return true;
		} catch (error: any) {
			showNotice({
	...
	const updateGroup = async (groupId: number, name: string) => {
		try {
			const response = (await apiFetch({
				path: `/doublescale/v1/custom-fields-groups/${groupId}`,
				method: 'PUT',
			...
			showNotice({
				type: 'success',
				message: __('Group updated', 'doublescale'),
			});
			return true;
		} catch (error: any) {
			showNotice({
	...
	const duplicateGroup = async (groupId: number, name?: string) => {
		try {
			const response = (await apiFetch({
				path: `/doublescale/v1/custom-fields-groups/${groupId}/duplicate`,
				method: 'POST',
				data: { name },
			})) as CustomFieldsGroup;
			...
			showNotice({
				type: 'success',
				message: __('Group duplicated successfully', 'doublescale'),
			});
			return true;
		} catch (error: any) {
			showNotice({
				...
				message:
					error.message ||
					__('Failed to duplicate group', 'doublescale'),
			});
			return false;
		}
	};
	...
	const deleteGroup = async (groupId: number, newGroupId?: number) => {
		try {
			...
			showNotice({
				type: 'success',
				message: __('Group deleted successfully', 'doublescale'),
			});
			return true;
		} catch (error: any) {
			showNotice({
				type: 'error',
				message:
					error.message || __('Failed to delete group', 'doublescale'),
			});
			return false;
		}
	};
