...
class Settings_Controller extends Abstract_Settings_Controller {
		...
		register_rest_route(
			...
			array(
				array(
					...
					'permission_callback' => array( $this, 'get_accounts_permissions_check' ),
					'args'                => array(),
				),
			)
		);
	 ...
	 */
	public function get_accounts( $request ) { // phpcs:ignore
		...
		try {
			...
			$result = $account_api->get_accounts();
			...
			return new \WP_Error( 'zoho_get_user_profile_error', $e->getMessage() );
		}
	}
	 ...
	 */
	public function get_accounts_permissions_check( $request ) {
