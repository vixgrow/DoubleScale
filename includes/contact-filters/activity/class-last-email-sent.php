...
/**
 * Last_Email_Sent class
 */
class Last_Email_Sent extends Filter {
		...
		switch ( $operator ) {
			...
			case 'between':
				$query->whereHas(
					'campaign_emails',
					function ( $query ) use ( $value ) {
						$query->whereBetween( 'sent_at', $value );
					}
				);
				break;
			case 'within':
				...
				$query->whereHas(
					'campaign_emails',
					function ( $query ) use ( $value ) {
						$query->whereDate( 'sent_at', '>=', $value[0] )
							->whereDate( 'sent_at', '<=', $value[1] );
					}
				);
				break;
		}
...
Filters_Manager::instance()->register( new Last_Email_Sent() );
