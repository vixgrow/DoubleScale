...
/**
 * Abstract_Curl_Multi_Mailer class
 *
 * @since 1.0.0
 */
abstract class Abstract_Curl_Multi_Mailer {
	...
	public function get_max_concurrent() {
		return apply_filters(
			'quillcrm_curl_multi_max_concurrent',
			$this->max_concurrent,
			$this->slug
		);
	}
	...
	public function get_max_batch_size() {
		return apply_filters(
			'quillcrm_curl_multi_max_batch_size',
			$this->max_batch_size,
			$this->slug
		);
	}
	...
	protected function log_send_attempt( $batch_data ) {
		quillcrm_get_logger()->info(
			/* translators: %s: mailer name */
			sprintf( __( 'Sending emails via %s cURL Multi', 'quillcrm' ), ucfirst( $this->slug ) ),
			array(
	...
	protected function log_send_success( $batch_data, $sent_count ) {
		quillcrm_get_logger()->info(
			__( 'cURL Multi email batch sent successfully', 'quillcrm' ),
			array(
	...
	protected function log_partial_failure( $batch_data, $results ) {
		quillcrm_get_logger()->warning(
			__( 'cURL Multi email batch partially failed', 'quillcrm' ),
			array(
