...
/** Cadence unit for a recurring invoice rule. */
export type RecurrenceUnit = 'day' | 'week' | 'month' | 'year';

/**
 * How a recurring series stops.
 *
 * The stored rule keeps `total_cycles` / `is_infinite` / `end_date` as separate
 * columns (either limit can end a series, whichever lands first), but the form
 * exposes one exclusive choice so a rule can never claim to run forever while
 * carrying a stop date.
 */
export type RecurrenceEndMode = 'never' | 'after_cycles' | 'on_date';

/**
 * A recurring-invoice rule attached to a template invoice (Pro).
 *
 * Present on the invoice payload only while the Recurring Invoices module is
 * active; `null` means the invoice does not recur.
 */
...
/** Payload accepted by the recurrence save endpoint. */
export interface InvoiceRecurrencePayload {
	enabled: boolean;
	interval_value?: number;
	interval_unit?: RecurrenceUnit;
	total_cycles?: number;
...
export interface Invoice {
	...
	balance?: number;
	is_overdue?: boolean;
	/** Set on invoices generated by a recurrence rule (Pro). */
	recurrence_id?: number | null;
	/** The rule this invoice is the template for (Pro). */
