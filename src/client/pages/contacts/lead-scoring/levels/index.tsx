...
import { NoticeBanner, NoData, CategoryIcon } from '@quillcrm/components';
...
const Levels = forwardRef<LevelsRef, LevelsProps>(({ activeTab }, ref) => {
	...
	const fetchLevels = async () => {
		...
		try {
			const response = (await apiFetch({
				path: addQueryArgs('/qc/v1/lead-scoring-levels', {
					per_page: perPage,
					page,
	...
	const validate = (level: Partial<LeadScoringLevel>) => {
		if (!level.name || level.name.trim() === '') {
			setVisible(false);
			showNotice('error', __('Level name is required', 'quillcrm'));
			return false;
		}
		if (level.points === undefined || level.points < 0) {
			setVisible(false);
			showNotice('error', __('Points must be a positive number', 'quillcrm'));
			return false;
		}
		return true;
	};
	...
	const createLevel = async () => {
		...
		try {
			await apiFetch({
				path: '/qc/v1/lead-scoring-levels',
				method: 'POST',
				data: level,
			});
			...
			setVisible(false);
			await fetchLevels();
			showNotice('success', __('Level created successfully', 'quillcrm'));
		} catch (error: any) {
			setVisible(false);
			showNotice('error', error.message);
		} finally {
	...
	const updateLevel = async () => {
		...
		try {
			const response = (await apiFetch({
				path: `/qc/v1/lead-scoring-levels/${selectedLevel?.id}`,
				method: 'PUT',
				data: selectedLevel,
			})) as LeadScoringLevel;
			...
			setVisible(false);
			setSelectedLevel(null);
			showNotice('success', __('Level updated successfully', 'quillcrm'));
		} catch (error: any) {
			setVisible(false);
			showNotice('error', error.message);
		} finally {
	...
	const deleteSelectedLevels = async () => {
		...
		try {
			...
			showNotice(
				'success',
				__('Selected levels deleted successfully', 'quillcrm')
			);
		} catch (error: any) {
			showNotice('error', error.message);
		} finally {
	...
	const tableConfig: DataTableConfig<LeadScoringLevel> = {
		...
		search: {
			placeholder: __('Search Levels', 'quillcrm'),
	...
	return (
		<div className="qcrm-lead-scoring-levels-list">
			{/* Notice Banner */}
			{notice && (
			...
			{loading || hasRecords ? (
				...
				<NoData
					...
					title={__('No Levels Found', 'quillcrm')}
					subtitle={__('Create your first lead scoring level to get started', 'quillcrm')}
					buttonLabel={__('Add Level', 'quillcrm')}
					onClick={() => {
						setSelectedLevel(null);
						setLevel({
							name: '',
