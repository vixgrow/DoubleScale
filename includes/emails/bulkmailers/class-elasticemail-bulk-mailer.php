<?php
/**
 * ElasticEmail Bulk Mailer
 *
 * Handles bulk email sending via ElasticEmail's API.
 *
 * @since 1.0.0
 * @package QuillCRM
...
/**
 * Elasticemail_Bulk_Mailer class
 *
 * @since 1.0.0
 */
class Elasticemail_Bulk_Mailer extends Abstract_Bulk_Mailer {
	 ...
	 * @var string
	 */
	protected $slug = 'elasticemail';

	/**
	 * Maximum recipients per batch (Elastic Email API v4 supports 1000)
	 *
	 * @var int
	 */
	...
	/**
	 * Convert QuillCRM merge tags to ElasticEmail merge field format
	 *
	 * ElasticEmail uses {field} syntax for merge fields.
	 * Converts {{contact:first_name}} to {first_name}
	 *
	 * @param string $content Content with QuillCRM merge tags
	 *
	 * @return string Content with ElasticEmail merge fields
	 */
	...
	/**
	 * Check if ElasticEmail is properly configured and available
	 *
	 * @return bool
	 */
	...
	/**
	 * Send a batch of emails via ElasticEmail
	 *
	...
	public function send_batch( $batch_data ) {
		...
		// Get ElasticEmail Account API
		$account_api = $this->get_account_api();

		if ( ! $account_api ) {
			return new WP_Error(
				'elasticemail_not_configured',
				__( 'ElasticEmail is not properly configured.', 'quillcrm' )
			);
		}
		...
		if ( ! method_exists( $account_api, 'send_batch' ) ) {
			return new WP_Error(
				...
				__( 'ElasticEmail batch sending is not available. Please update QuillSMTP.', 'quillcrm' )
			);
		}
		...
		// Prepare batch args for ElasticEmail
		$elasticemail_args = array(
		...
		if ( ! empty( $batch_data['text'] ) ) {
			$elasticemail_args['text'] = $batch_data['text'];
		}

		// Add reply-to
		if ( ! empty( $batch_data['reply_to'] ) ) {
			$elasticemail_args['reply_to'] = $batch_data['reply_to'];
		}

		// Add tags for campaign tracking
		if ( ! empty( $batch_data['tags'] ) ) {
			$elasticemail_args['tags'] = $batch_data['tags'];
		} elseif ( ! empty( $batch_data['campaign_id'] ) ) {
			$elasticemail_args['tags'] = array( 'campaign-' . $batch_data['campaign_id'] );
		}
		...
		$result = $account_api->send_batch( $elasticemail_args );
	...
	/**
	 * Build headers for ElasticEmail request
	 *
	 * @param array $batch_data Batch email data
	 *
	...
	/**
	 * Get the ElasticEmail Account API instance
	 *
	...
	private function get_account_api() {
		...
		if ( ! class_exists( '\\QuillSMTP\\Mailers\\ElasticEmail\\Accounts' ) ) {
			return null;
		}
		...
		if ( ! empty( $settings['default_connection'] ) ) {
			...
			if ( isset( $connections[ $default_connection_id ] ) ) {
				...
				if ( ! empty( $connection['mailer'] ) && $connection['mailer'] === 'elasticemail' ) {
					$account_id = $connection['account_id'] ?? null;
				}
			}
		}

		// Check fallback_connection if default is not ElasticEmail
		...
		try {
			...
			$elasticemail = $mailers->get_mailer( 'elasticemail' );

			if ( ! $elasticemail || ! isset( $elasticemail->accounts ) ) {
				return null;
			}

			$account_api = $elasticemail->accounts->connect( $account_id );
			...
			quillcrm_get_logger()->error(
				__( 'Failed to get ElasticEmail Account API', 'quillcrm' ),
				array(
					'code'  => 'bulk_email_elasticemail_api_error',
					'error' => $e->getMessage(),
				)
			);
			return null;
