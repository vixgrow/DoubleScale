...
abstract class Accounts {
	...
	public function connect( $account_id ) {
		if ( ! isset( $this->account_apis[ $account_id ] ) ) {
			...
			if ( ! isset( $accounts_data[ $account_id ] ) ) {
				return new WP_Error(
					"doublescale_smtp_{$this->provider->slug}_cannot_find_account_data",
					sprintf(
						/* translators: 1: account id stored on the connection, 2: mailer/provider slug */
						__( 'Cannot find account data for account "%1$s" (mailer: %2$s). Re-authorize this provider in SMTP settings or pick a valid account on the connection.', 'doublescale' ),
						esc_html( (string) $account_id ),
						esc_html( (string) $this->provider->slug )
					),
					array(
						'account_id' => $account_id,
						'mailer'     => $this->provider->slug,
					)
				);
			}
			// init account api.
			try {
				...
				return new WP_Error(
					"doublescale_smtp_{$this->provider->slug}_cannot_init_account_api",
					esc_html__( 'Cannot connect to account api', 'doublescale' )
				);
			}
		}
	...
	final protected function get_accounts_data() {
		do_action( 'doublescale_smtp_before_get_mailers_settings' );
		$account = $this->provider->settings->get( 'accounts' ) ?? array();
		do_action( 'doublescale_smtp_after_get_mailers_settings' );
	...
	final protected function add_account_data( $account_id, $account_data ) {
		...
		if ( isset( $data[ $account_id ] ) ) {
			doublescale_get_logger()->error(
				esc_html__( 'Account id already exists', 'doublescale' ),
				array(
	...
	final protected function update_account_data( $account_id, $account_data ) {
		...
		if ( ! isset( $data[ $account_id ] ) ) {
			doublescale_get_logger()->error(
				esc_html__( 'Cannot find account to update', 'doublescale' ),
				array(
					'source'     => static::class . '->' . __FUNCTION__,
