...
use DoubleScale\Vendor\Psr\Cache\CacheItemInterface;
use DoubleScale\Vendor\Psr\Cache\CacheItemPoolInterface;
use DoubleScale\Vendor\Symfony\Component\Cache\DoctrineProvider as SymfonyDoctrineProvider;
...
final class CacheAdapter implements CacheItemPoolInterface
{
    ...
    {
        if ($cache instanceof \Doctrine\Common\Cache\Psr6\DoctrineProvider && !$cache->getNamespace()) {
    ...
    public function getItem($key) : CacheItemInterface
    {
        ...
        if (PHP_VERSION_ID >= 80000) {
            if ($value !== \false) {
                return new \Doctrine\Common\Cache\Psr6\TypedCacheItem($key, $value, \true);
            }
            return new \Doctrine\Common\Cache\Psr6\TypedCacheItem($key, null, \false);
        }
        if ($value !== \false) {
            return new \Doctrine\Common\Cache\Psr6\CacheItem($key, $value, \true);
        }
        return new \Doctrine\Common\Cache\Psr6\CacheItem($key, null, \false);
    }
    /**
     * {@inheritDoc}
     */
    public function getItems(array $keys = []) : array
    {
        ...
        if (PHP_VERSION_ID >= 80000) {
            foreach ($keys as $key) {
                if (array_key_exists($key, $values)) {
                    $items[$key] = new \Doctrine\Common\Cache\Psr6\TypedCacheItem($key, $values[$key], \true);
                } else {
                    $items[$key] = new \Doctrine\Common\Cache\Psr6\TypedCacheItem($key, null, \false);
                }
            }
            return $items;
        }
        foreach ($keys as $key) {
            if (array_key_exists($key, $values)) {
                $items[$key] = new \Doctrine\Common\Cache\Psr6\CacheItem($key, $values[$key], \true);
            } else {
                $items[$key] = new \Doctrine\Common\Cache\Psr6\CacheItem($key, null, \false);
            }
        }
        return $items;
    }
    ...
    {
        if (!$item instanceof \Doctrine\Common\Cache\Psr6\CacheItem && !$item instanceof \Doctrine\Common\Cache\Psr6\TypedCacheItem) {
            return \false;
        }
    ...
    {
        if (!is_string($key)) {
            throw new \Doctrine\Common\Cache\Psr6\InvalidArgument(sprintf('Cache key must be string, "%s" given.', is_object($key) ? get_class($key) : gettype($key)));
        }
        if ($key === '') {
            throw new \Doctrine\Common\Cache\Psr6\InvalidArgument('Cache key length must be greater than zero.');
        }
        if (strpbrk($key, self::RESERVED_CHARACTERS) !== \false) {
            throw new \Doctrine\Common\Cache\Psr6\InvalidArgument(sprintf('Cache key "%s" contains reserved characters "%s".', $key, self::RESERVED_CHARACTERS));
        }
        return \true;
    }
    /**
