...
class UpdateContactFields extends Action {
	...
	private function normalize_for_column( $column, $value ) {
		...
		switch ( $column ) {
			...
			case 'phone':
				$normalized = $this->normalize_phone( $value, false );
				return ( null !== $normalized && preg_match( '/^\+?[0-9]+$/', $normalized ) ) ? $normalized : null;
	...
	/**
	 * Strip a phone string down to an optional leading "+" plus digits.
	 *
	 * @since 1.0.0
	 *
	 * @param string $value        Raw phone value.
	 * @param bool   $require_plus When true, a value without a leading "+" is rejected (E.164).
	 *
	 * @return string|null
	 */
	private function normalize_phone( $value, $require_plus ) {
		if ( '' === $value ) {
			return null;
		}
