...
namespace QuillCRM\Emails\CurlMulti;

use WP_Error;

/**
 * SMTP2GO_Curl_Multi_Mailer class
 *
 * @since 1.0.0
 */
class SMTP2GO_Curl_Multi_Mailer extends Abstract_Curl_Multi_Mailer {
	...
	/**
	 * Get API credentials from QuillSMTP settings
	 *
	...
	protected function get_credentials() {
		return $this->get_account_credentials( 'api_key' );
	}
	...
	/**
	 * Override send_chunk to use QuillSMTP Account_API for sending and logging
	 *
	...
	protected function send_chunk( $recipients, $batch_data, $credentials ) {
		...
		// Use QuillSMTP Account_API for sending with logging capability
		$account_api = new \QuillSMTP\Mailers\SMTP2GO\Account_API( $credentials['api_key'] );
		...
		$log_callback = function( $email, $request_data, $response, $http_code, $error, $type ) {
			 ...
			 */
			do_action( 'quillcrm_smtp2go_curl_multi_log', $email, $request_data, $response, $http_code, $error, $type );
		};
	 ...
	 *
	 * Delegates to QuillSMTP Account_API for consistent response parsing.
	 *
	...
	protected function parse_response( $response, $http_code ) {
		...
		$account_api = new \QuillSMTP\Mailers\SMTP2GO\Account_API( $api_key );
