...
namespace DoubleScale\Vendor\ArPHP\MZK;
...
class Speller
{
    ...
    public function __construct(array $options = [])
    {
        ...
        try {
            $this->conn = new \PDO('sqlite:' . $tmpfile_path, null, null, [\PDO::ATTR_PERSISTENT => \true]);
            $this->conn->setAttribute(\PDO::ATTR_ERRMODE, \PDO::ERRMODE_EXCEPTION);
        } catch (\PDOException $e) {
            echo "Connection failed: " . $e->getMessage();
        }
        $data = \json_decode(\file_get_contents($this->rootDirectory . "/data/SarahSpell/confusion.json"), \true);
    ...
    function spell_check($r, $with_suggests = \false, $second_language = 'en', $use_stemmer = \false)
    {
        ...
        foreach ($words as $wi => $word) {
            if ($this->is_english($this->strip_tashkeel($word)) and $this->pspell_link != null) {
                if (!\pspell_check($this->pspell_link, $word)) {
                    ...
                    $suggestion_array[] = ['word' => $word, 'stemming' => [], 'sugg' => $with_suggests ? $suggestions : [], 'context' => ''];
            ...
            $word = \trim($this->clean_word($this->strip_tashkeel($word)));
            ...
            if (!$in_corpus) {
                ...
                if (!$stem_hit and !$prefix_hit and !isset($displayed_cache[$word]) and \count($sgg) > 0) {
                    ...
                    $suggestion_array[] = ['word' => $word, 'stemming' => ['pr' => @$stemmed_word['pr'], 'root' => @$stemmed_word['root'], 'su' => @$stemmed_word['su']], 'sugg' => $y, 'context' => $context];
                    @$displayed_cache[$word]++;
                } else {
                    if (!$stem_hit and !$prefix_hit and isset($displayed_cache[$word])) {
                        @$displayed_cache[$word]++;
                    }
                }
                if (\count($sgg) == 0 and !$stem_hit and !$prefix_hit) {
                    ...
                    if (!\in_array($word, \array_keys($no_sugg_cache))) {
                        $context = "";
                        $word_sugg = 'no sugg';
                        $no_sugg_cache[$word] = ['word' => $word, 'stemming' => ['pr' => @$stemmed_word['pr'], 'root' => @$stemmed_word['root'], 'su' => @$stemmed_word['su']], 'context' => $context];
                    }
                }
            }
            $prev_word = $word;
        ...
        $return_array = ["suggestion_array" => $suggestion_array, "displayed_cache" => $displayed_cache, "no_sugg_cache" => $no_sugg_cache, "stemmed_words" => $stemmed_words, "stemmed_sugg" => $stemmed_sugg];
        return $return_array;
    }
    /* begin functions */
    // -----------
    ...
    private function suggestions_for_word($word, $prev_word)
    {
        ...
        foreach ($sugg as $sg) {
            if ($this->prepare_query_corpus(@$sg['sugg'], \true)) {
    ...
    private function generate_suggestions($arr)
    {
        foreach ($arr as $vv) {
            ...
            $b = \array_merge($this->replace_variate($splits, $letters), $this->transpose_variate($splits), $this->del_variate($splits), $this->ins_variate_1($splits));
            return $b;
        }
    }
    // -----
    ...
    {
        $tashkeel = array("ّ", "ْ", "ٍ", "ِ", "ٌ", "ُ", "ً", "َ");
