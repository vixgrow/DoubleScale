 ...
 * HTTP Request Webhook Action
 */
class Http_Request_Webhook extends Action {
	 ...
	 * @var string
	 */
	public $name = 'Send to HTTP Request Webhook';
	...
	public function process_action( Automation_Model $automation, Automation_Step_Model $step, Automation_Contact_Model $automation_contact ) {
		...
		if ( empty( $webhook_url ) ) {
			quillcrm_get_logger()->error(
				\__( 'HTTP Request Webhook action is missing webhook URL.', 'quillcrm' ),
				array(
					'code' => 'http_request_webhook_missing_url',
					'data' => array(
						'automation' => array(
							'id'   => $automation->id,
		...
		if ( ! \filter_var( $webhook_url, \FILTER_VALIDATE_URL ) ) {
			quillcrm_get_logger()->error(
				\__( 'HTTP Request Webhook action has invalid webhook URL.', 'quillcrm' ),
				array(
					'code' => 'http_request_webhook_invalid_url',
					'data' => array(
						'automation'  => array(
							'id'   => $automation->id,
							'name' => $automation->name,
		...
		if ( ! empty( $headers_fields ) && is_array( $headers_fields ) ) {
			if ( isset( $headers_fields[0] ) && is_array( $headers_fields[0] ) && isset( $headers_fields[0]['key'] ) ) {
				foreach ( $headers_fields as $pair ) {
					if ( ! empty( $pair['key'] ) && ! empty( $pair['value'] ) ) {
						// Process merge tags in the header value
						$processed_value                = $this->merge_tags_manager->process_merge_tags( $pair['value'], $automation_contact );
						$custom_headers[ $pair['key'] ] = $processed_value;
					}
				}
			}
		}
		...
		if ( \is_wp_error( $response ) ) {
			quillcrm_get_logger()->error(
				\__( 'HTTP Request Webhook action failed to send data.', 'quillcrm' ),
				array(
					'code' => 'http_request_webhook_request_failed',
					'data' => array(
						'automation'  => array(
							'id'   => $automation->id,
							'name' => $automation->name,
		...
		if ( $response_code < 200 || $response_code >= 300 ) {
			quillcrm_get_logger()->error(
				\__( 'HTTP Request Webhook action received error response.', 'quillcrm' ),
				array(
					'code' => 'http_request_webhook_error_response',
					'data' => array(
						'automation'    => array(
							'id'   => $automation->id,
							'name' => $automation->name,
		...
		quillcrm_get_logger()->info(
			\__( 'HTTP Request Webhook action successfully sent data.', 'quillcrm' ),
			array(
				'code' => 'http_request_webhook_success',
				'data' => array(
					'automation'    => array(
						'id'   => $automation->id,
						'name' => $automation->name,
	...
	public function get_fields() {
		return array(
			'webhook_url'    => array(
				...
				'description' => \__( 'Enter your webhook URL', 'quillcrm' ),
			),
			'method'         => array(
				'type'        => 'select',
				'label'       => \__( 'Method', 'quillcrm' ),
				'description' => \__( 'Select the HTTP method to use for the request.', 'quillcrm' ),
				'options'     => array(
					'POST' => 'POST',
					'GET'  => 'GET',
					'PUT'  => 'PUT',
			...
			'headers_fields' => array(
				'type'        => 'dynamic_keyvalue',
				'label'       => \__( 'Headers', 'quillcrm' ),
				'description' => \__( 'Add custom headers to send to the webhook. Keys will be used as header names in the payload.', 'quillcrm' ),
			),
			'test_webhook'   => array(
				'type'        => 'button',
				...
				'settings'    => array(
					'ajax_action' => 'quillcrm_test_http_request_webhook',
	...
	public function register() {
		...
		\add_action( 'wp_ajax_quillcrm_test_http_request_webhook', array( $this, 'ajax_test_webhook' ) );
	}
		...
		if ( ! empty( $headers_fields ) && \is_array( $headers_fields ) ) {
			foreach ( $headers_fields as $pair ) {
				if ( ! empty( $pair['key'] ) && ! empty( $pair['value'] ) ) {
					...
					$test_headers[ \sanitize_text_field( $pair['key'] ) ] = \sanitize_text_field( $pair['value'] );
				}
			}
		}
