...
class Import {
	...
	private function import_with_offset( $total, $offset, $get_subscribers_callback, $mapping ) {
		...
		// Suppress per-contact notifications during batch import (e.g., contact_subscribed broadcasts).
		// The import_completed notification fires after resume.
		\DoubleScale\Modules\Notifications\Services\NotificationService::suppress();
		...
		try {
			...
			// Always resume, even if an exception interrupts the loop.
			\DoubleScale\Modules\Notifications\Services\NotificationService::resume();
		}
