...
import { Loader2, Mail, Phone, User, Calendar, Tag, List } from 'lucide-react';

// Import types
import { Contact } from '@quillcrm/client';
	...
	const fetchSubscribers = async () => {
		...
		try {
			...
			createNotice({
				...
				message:
					error.message ||
					__('Failed to fetch subscribers', 'quillcrm'),
			});
		} finally {
			setLoading(false);
		}
			...
			<DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
				<DialogHeader className="flex-shrink-0">
					<DialogTitle className="flex items-center gap-2">
						...
						{__('Subscribers for', 'quillcrm')} "{sequenceName}"
						<Badge variant="secondary" className="ml-2">
							{subscribers.length} {__('subscribers', 'quillcrm')}
						</Badge>
					</DialogTitle>
				</DialogHeader>
				...
				<div className="flex-1 overflow-y-auto">
					{loading ? (
						<div className="flex items-center justify-center py-8">
							...
							<span className="ml-2">
								{__('Loading subscribers...', 'quillcrm')}
							</span>
						</div>
