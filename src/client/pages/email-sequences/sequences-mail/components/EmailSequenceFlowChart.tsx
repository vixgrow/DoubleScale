...
// 	return (
// 		<div 
// 			className={`bg-white rounded-l-lg ${isHovered?'rounded-br-lg' :'rounded-r-lg'}   border-2 border-[#458DC7] py-[14px] px-4 min-w-80 max-w-80 flex flex-col gap-3 shadow-sm select-none relative group`}
...
// 				<span className="text-sm font-semibold text-[#09090B]">{__(`Step ${nodeData.number}`,'quillcrm')}</span>
...
//                 </span>
// 				<span className=" leading-5 text-[#458DC7] truncate font-bold text-xl">{nodeData.subject || nodeData.name || 'Untitled Email'}</span>
// 			</div>
// 		</div>
// 	);
// };
...
// const StartNode = () => {
// 	return (
// 		<div className="bg-white border border-[#DEE1E6] rounded-lg py-[14px] px-4 select-none ">
// 			<Handle
// 				type="source"
// 				position={Position.Bottom}
// 				className="!w-3 !h-3 !bg-white !border-2 !border-gray-400 opacity-0"
...
//                 </span>
// 				<span className="text-xl font-bold leading-[30px] text-[#09090B]">{__('Start (Email Sequence)', 'quillcrm')}</span>
// 			</div>
// 		</div>
// 	);
// };
...
// 			// If no emails, just show edge with + button from start
// 			// No end node needed
// 			newEdges.push({
// 				id: 'edge-start-placeholder',
// 				source: 'start',
// 				target: 'start', // Self-loop will be handled by custom positioning
// 				type: 'custom',
// 				data: {
...
// 				target: `email-${lastEmail.id}`, // Self-reference for positioning
// 				type: 'custom',
// 				data: {
...
// 		setNodes(newNodes);
// 		setEdges(newEdges);
// 	}, [sequenceEmails, loading, onAddEmail, onEditEmail, onDeleteEmail, onDuplicateEmail, onShowReport]);

// 	if (loading) {
// 		return (
// 			<div className="h-full flex items-center justify-center bg-[#F6F8FA]">
// 				<p className="text-xl text-gray-600">{__('Loading sequence...', 'quillcrm')}</p>
// 			</div>
// 		);
// 	}
	...
	useEffect(() => {
		...
		if (sequenceEmails.length === 0) {
			...
			newNodes.push({
				id: 'invisible-end',
				type: 'invisibleEnd',
				position: { x: HORIZONTAL_POS, y: 150 },
				data: {},
				draggable: false,
				selectable: false,
			});
