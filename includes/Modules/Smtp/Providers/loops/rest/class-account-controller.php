...
namespace DoubleScale\Modules\Smtp\Providers\Loops\REST;
...
class Account_Controller extends Abstract_Account_Controller {
	...
	protected function get_account_info( $request ) {
		...
		if ( empty( $api_key ) ) {
			return new WP_Error( 'doublescale_smtp_loops_api_key_missing', __( 'API key is missing.', 'doublescale' ) );
		}

		if ( empty( $transactional_id ) ) {
			return new WP_Error( 'doublescale_smtp_loops_transactional_id_missing', __( 'Transactional ID is missing.', 'doublescale' ) );
		}
		...
		if ( ! isset( $data['success'] ) || $data['success'] !== true ) {
			return new WP_Error( 'doublescale_smtp_loops_api_key_invalid', __( 'Invalid API key.', 'doublescale' ) );
		}
