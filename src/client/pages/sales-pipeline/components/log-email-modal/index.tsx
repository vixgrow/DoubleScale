	...
	const handleSubmit = async (values: EmailFormData) => {
		...
		try {
			...
			onSuccess({
				type: 'success',
				message: editMode 
				  ? __('Email updated successfully!', 'quillcrm')
				  : __('Email logged successfully!', 'quillcrm'),
			  });
			onClose();
			form.reset();
		} catch (error) {
			...
			setNotice({
				type: 'error',
				message: err.message || __(editMode ? 'Failed to update email' : 'Failed to logged email', 'quillcrm'),
			});
		} finally {
			setLoading(false);
		}
