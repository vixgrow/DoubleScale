...
use DoubleScale\Modules\Forms\MergeTags\Forms\DynamicFieldsRegistration;
use DoubleScale\Modules\Automations\Rules\Forms\FormFieldRule;
...
abstract class Form {
	...
	public function register_merge_tags_for_form( $form_id, array $new_fields = array() ) {
		...
		new DynamicFieldsRegistration( $fields, $this->slug );
	}
	...
	public function register_field_rules_for_form( $form_id, array $new_fields = array() ): void {
		...
		foreach ( $fields as $field_id => $field_name ) {
			$rule = new FormFieldRule( $this, $form_id, $field_id, $field_name['label'] ?? '' );
	...
	public function process_form( $data ) {
		...
		Plugin::instance()->forms_tasks->enqueue_async( 'process_form', $data );
	}
	...
	public function process_form_sync( $data ) {
		try {
			...
			// Send double opt-in email if enabled and email is unverified
	...
	public function process_automations( $args ) {
		try {
			...
			foreach ( $automations as $automation ) {
				...
				Plugin::instance()->automations_tasks->enqueue_sync( 'process_automations', $automation, $data );
			}
		} catch ( Exception $e ) {
			doublescale_get_logger()->error(
	...
	public function maybe_create_contact( AutomationModel $automation ) {
		try {
			...
			if ( ! empty( $custom_fields ) && class_exists( 'DoubleScale\Core\CustomFields\Models\CustomFieldModel' ) ) {
				$custom_fields_values = array();
				foreach ( $custom_fields as $key => $value ) {
					$custom_fields_values[ $key ] = array(
						'value'       => $value,
	...
	public function restore_form_data( $settings ) {
		...
		// Create a FormModel instance from the settings array
