...
class Emails {
	...
	public function get_theme_template_paths() {
		...
		$file_paths = array(
			...
			100 => DOUBLESCALE_PLUGIN_DIR . 'includes/Modules/Campaigns/Emails/Templates',
		);
	...
	public static function send_double_optin_email( $contact ) {
		try {
			$settings   = \DoubleScale\Pro\Settings::get( 'double_optin', array() );
			$subject    = $settings['email_subject'] ?? \DoubleScale\Pro\Settings::get_default_opt_in_subject();
			$content    = $settings['email_content'] ?? \DoubleScale\Pro\Settings::get_default_opt_in_content();
