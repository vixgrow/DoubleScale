...
export class UserService {
	 ...
	 */
	private static buildCRMSearchParams(
		options: UserSearchOptions
	): URLSearchParams {
		const mergedOptions = { ...this.DEFAULT_OPTIONS, ...options };
		const params = new URLSearchParams();
	...
	private static transformCRMUser(crmUser: any): User {
		return {
			...
			display_name:
				crmUser.display_name || crmUser.name || `User ${crmUser.id}`,
			email: crmUser.email || '',
			name: crmUser.name || crmUser.display_name,
			username: crmUser.username,
			avatar_urls: crmUser.avatar_urls,
		};
	}
	...
	static async getUsers(options: UserSearchOptions = {}): Promise<User[]> {
		try {
			...
			const users = Array.isArray(apiResponse.users)
				? apiResponse.users
				: [];
	 ...
	 */
	private static async getUsersFromWordPress(
		options: UserSearchOptions = {}
	): Promise<User[]> {
		try {
	...
	static searchUsers = debounce(
		async (
			searchTerm: string,
			options: Omit<UserSearchOptions, 'searchTerm'> = {}
		): Promise<User[]> => {
	 ...
	 */
	static async getUsersForSelect(
		options: UserSearchOptions = {}
	 ...
	 */
	static async getUsersForReports(
		options: UserSearchOptions = {}
