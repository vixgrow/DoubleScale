...
/**
 * Postmark_Bulk_Mailer class
 *
 * @since 1.0.0
 */
class Postmark_Bulk_Mailer extends Abstract_Bulk_Mailer {
	...
	/**
	 * Convert QuillCRM merge tags to Postmark format
	 *
	...
	public function send_batch( $batch_data ) {
		...
		// Get Postmark Account API
		$account_api = $this->get_account_api();

		if ( ! $account_api ) {
			return new WP_Error(
				'postmark_not_configured',
				__( 'Postmark is not properly configured.', 'quillcrm' )
			);
		}
		...
		if ( ! method_exists( $account_api, 'send_batch' ) ) {
			return new WP_Error(
				...
				__( 'Postmark batch sending is not available. Please update QuillSMTP.', 'quillcrm' )
			);
		}
	...
	/**
	 * Get the Postmark Account API instance
	 *
	...
	private function get_account_api() {
		...
		if ( ! class_exists( '\\QuillSMTP\\Mailers\\PostMark\\Accounts' ) ) {
			return null;
		}
		...
		if ( ! empty( $settings['default_connection'] ) ) {
			...
			if ( isset( $connections[ $default_connection_id ] ) ) {
				...
				if ( ! empty( $connection['mailer'] ) && $connection['mailer'] === 'postmark' ) {
					$account_id = $connection['account_id'] ?? null;
				}
			}
		}

		// Check fallback_connection if default is not Postmark
		...
		try {
			...
			$postmark = $mailers->get_mailer( 'postmark' );

			if ( ! $postmark || ! isset( $postmark->accounts ) ) {
				return null;
			}

			$account_api = $postmark->accounts->connect( $account_id );
			...
			quillcrm_get_logger()->error(
				__( 'Failed to get Postmark Account API', 'quillcrm' ),
				array(
					'code'  => 'bulk_email_postmark_api_error',
					'error' => $e->getMessage(),
				)
			);
			return null;
