...
const EditSequenceMail: React.FC<EditSequenceMailProps> = ({
	...
	useEffect(() => {
		if (isEditing && emailId) {
			// Reset emailData when switching to a different email
			setEmailData(null);
			setNotice(null);
			fetchEmailData();
		} else if (!isEditing) {
	...
	const fetchEmailData = async () => {
		...
		try {
			...
			setNotice({
				...
				message:
					error.message ||
					__('Failed to fetch email data', 'doublescale'),
			});
	...
	const handleSave = async (data: SequenceMailFormData) => {
		...
		try {
			...
			setNotice({
				type: 'success',
				message: __('Sequence email updated successfully', 'doublescale'),
			});
			...
			setNotice({
				...
				message:
					error.message ||
					__('Failed to update sequence email', 'doublescale'),
			});
		} finally {
			setLoading(false);
		}
	...
	return emailData ? (
		<SequenceMailModal
			isOpen={isEditing}
			onClose={handleClose}
			title={__('Edit Sequence Email', 'doublescale')}
			onSave={handleSave}
			initialData={{
