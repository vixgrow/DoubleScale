...
import { CustomField, FieldDialogProps, NoticeMessage } from '@doublescale/client';
	...
	currentScope = 'contact',
		...
		const handleSubmit = async () => {
			if (
				...
				showNotice({
					type: 'error',
					message: __('Please fill all the required fields', 'doublescale'),
				});
			...
			if (needsOptions) {
				...
				if (validOptions.length === 0) {
					showNotice({
						type: 'error',
						message: __('Please add at least one option', 'doublescale'),
					});
					setIsSubmitting(false);
					return;
				}
		...
		return (
			<Dialog open={visible} onOpenChange={onClose}>
				<DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
					...
					<div className="doublescale-fields space-y-3">
						<Field
							...
							type="text"
							placeholder={__('Enter Field Name', 'doublescale')}
						/>
						...
						<Field
							...
							placeholder={__('Select Field Type', 'doublescale')}
						/>
						...
						<Field
							...
							placeholder={__('Select Field Group', 'doublescale')}
						/>
						...
						{needsOptions && (
							<div className="space-y-2">
								<label className="text-sm font-medium text-gray-900">
									{__('Field Value Options', 'doublescale')}
								</label>
								<div className="space-y-2">
