...
class Logger implements Logger_Interface {
	 ...
	 *
	 * @param array  $handlers Optional. Array of log handlers. If $handlers is not provided, the filter 'quillcrm_register_log_handlers' will be used to define the handlers. If $handlers is provided, the filter will not be applied and the handlers will be used directly.
	...
	public function __construct( $handlers = null, $threshold = null ) {
		if ( null === $handlers ) {
			$handlers = apply_filters( 'quillcrm_register_log_handlers', array() );
		}
	...
	public function add( $handle, $message, $level = Log_Levels::NOTICE ) {
		$message = apply_filters( 'quillcrm_logger_add_message', $message, $handle );
		$this->log(
			$level,
			$message,
			array(
	...
	public function log( $level, $message, $context = array() ) {
		...
		if ( $this->should_handle( $level ) ) {
			$timestamp = time();
			$message   = apply_filters( 'quillcrm_logger_log_message', $message, $level, $context );
	...
	public function clear_expired_logs() {
		$days      = absint( apply_filters( 'quillcrm_logger_days_to_retain_logs', 30 ) );
