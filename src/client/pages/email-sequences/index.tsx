...
const EmailSequences: React.FC = () => {
	...
	const fetchSequences = async () => {
		...
		try {
			...
			createNotice({
				...
				message:
					error.message ||
					__('Failed to fetch email sequences', 'quillcrm'),
			});
		} finally {
			setLoading(false);
		}
	...
	const handleDelete = async (id: number) => {
		try {
			...
			createNotice({
				type: 'success',
				message: __('Email sequence deleted successfully', 'quillcrm'),
			});
			...
			createNotice({
				...
				message:
					error.message ||
					__('Failed to delete email sequence', 'quillcrm'),
			});
		}
	};
	...
	const handleEdit = (id: number, _name: string) => {
	...
	return (
		<div className="qcrm-email-sequences">
			<PageHeader
				title={__('Email Sequences', 'quillcrm')}
				subtitle={__('Manage your email sequences', 'quillcrm')}
				actions={[
					{
						label: __('Create Sequence', 'quillcrm'),
						icon: <PlusIcon />,
